...
하나의 노드가 2개의 값을 가지고 있으면서 3개의 자식 노드를 가지는 트리

## 2-3 검색 트리의 검색 과정

찾고자 하는 값이 노드가 가진 2개의 값보다 작으면 왼쪽 노드, 2개의 값 사이면 가운데 노드, 크면 오른쪽 노드로 검색

## 삽입

만약 어떤 노드가 3개의 값을 가져야 한다면 가운데 값이 부모 노드가 되고 나머지 2개의 값이 각각 왼쪽, 오른쪽 노드로 변환
...
* 트리의 높이가 최선의 경우 밑을 3으로 하는 logN이 된다.
* 최선, 최악의 경우 모두 O(logN)의 시간복잡도를 갖는다.
